...
Vec3f Mesh::setupHelperForColor(Face *f, int i, int j) {
  assert (getFace(i) == f);
  assert (j >= 0 && j < 4);
...
void Mesh::setupVBOs() {
  ...
  for (int i = 0; i < num_faces; i++) {
    ...
    for (int j = 0; j < 4; j++) {
      ...
      color = Vec3f(linear_to_srgb(color.r()),linear_to_srgb(color.g()),linear_to_srgb(color.b()));
  ...
  glBufferData(GL_ARRAY_BUFFER,sizeof(VBOPosNormalColorTexture) * num_faces * 4,&mesh_quad_verts[0],GL_STATIC_DRAW); 
